        ...
        IEnumerator Targeting(AbilityData data, PlayerController playerController, Action finished)
        {
            ...
            while (!data.IsCanelled())
            {
                ...
                if (Physics.Raycast(PlayerController.GetMouseRay(), out rayCastHit, 1000, layerMask))
                {
                    ...
                    {
                        // Prevents character from moving when you activate the ability
                        // because for a few frames the mouse will be held down which will activate player movement.
                        // this will wait until the mouse button is released before reenabling player control.
